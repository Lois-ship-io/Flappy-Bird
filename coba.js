...
enemyImg.src = "img/enemy.png"; // Ganti dengan path gambar musuh
let enemyArray = [];
let enemyWidth = 40;
let enemyHeight = 40;
let enemySpeed = -3;
...
function update() {
  ...
  for (let i = 0; i < pipeArray.length; i++) {
    ...
    // Spawn enemy every multiple of 6
    if (Math.floor(score) % 6 === 0 && Math.floor(score) !== 0) {
      spawnEnemy();
    }
  }
  ...
  updateEnemies(); // Panggil fungsi updateEnemies
...
function spawnEnemy() {
  if (enemyArray.length < 3) {
    ...
    let enemy = {
      ...
      height: enemyHeight,
      speedX: enemySpeed * 2,
      speedY: (Math.random() < 0.5 ? 1 : -1) * (enemySpeed * 0.5),
    };
    enemyArray.push(enemy);
  }
}

// Fungsi untuk Mengupdate dan Menggambar Musuh
function updateEnemies() {
  for (let i = 0; i < enemyArray.length; i++) {
    ...
    if (enemy.y <= 0 || enemy.y >= boardHeight - enemyHeight) {
